...
SIMData <- function (sameSigma = TRUE, sameLambda = FALSE, p, q, K.true, 
    n, loading_means, loading_sd, sINV_values) 
{
    if (missing(p)) {
        p = 40
    }
    ...
    if (missing(sINV_values)) {
        if (sameSigma) {
            ...
            sINV_values = matrix(rgamma(K.true * p, shape = 1, 
                rate = 1), nrow = K.true, ncol = p)
        }
    }
    if (missing(loading_means)) {
        loading_means <- c(-30, -20, -10, 10, 20, 30)
    }
    if (missing(loading_sd)) {
        loading_sd <- rep(2, length(loading_means))
    }
    ...
    if (q >= p) {
        stop("q should not be greater than p")
    }
    cat(paste0("   n = ", n), "\n")
    cat(paste0("   p = ", p), "\n")
    cat(paste0("   q = ", q), "\n")
    ...
    if (sameLambda == FALSE) {
        Lambda.true <- array(data = rnorm(K.true * p * q, mean = 0, 
            sd = 1), dim = c(K.true, p, q))
    }
    else {
        Lambda.true <- array(data = rnorm(K.true * p * q, mean = 0, 
            sd = 1), dim = c(K.true, p, q))
    ...
    for (k in 1:K.true) {
        ...
        for (j in 1:q) {
            ...
            if (sameLambda == FALSE) {
                Lambda.true[k, , j] <- rnorm(p, mean = meanCOL, 
                  sd = loading_sd[pickAnIndex])
#                if (j > 1) {
#                  Lambda.true[k, 1:(j - 1), j] <- rep(0, j - 
#                    1)
#                }
            }
            else {
 ...
 #                 Lambda.true[k, 1:(q-1), j] <- rep(0, q-1)	# logika tha exei fa switch edw.
                if (k > 1) {
        ...
        if (u < 1/3) {
            ...
            if (u < 2/3) {
                ...
                mu.true[k, ] <- 40 * (sin(seq(0, k * pi, length = p)))^2 - 
                  40 * (cos(seq(0, k * pi, length = p)))^2
            }
        }
    }
    if (sameSigma == TRUE) {
    ...
    for (i in 1:n) {
        ...
        j <- z.true[i]
        if (q == 1) {
            x_mean <- mu.true[j, ] + Lambda.true[j, , ] %*% array(y.true[i, 
                ], dim = c(q, 1))
        }
        else {
    ...
    legend("bottomleft", paste0("cluster ", 1:K.true, ": ", as.character(as.numeric(table(z.true)))), 
        col = 1:K.true, lty = 1)
    ...
    results[[6]] <- y.true
    results[[7]] <- w.true
    names(results) <- c("data", "class", "factorLoadings", "means", 
        "variance", "factors", "weights")
    return(results)
}
...
ledermann <- function(p){(2*p+1-sqrt(8*p+1))/2}
